...
function dwell(
  element: Element,
  spec: { rise: number; touchRise?: number; fall: number; drain?: number },
): Dwell;
interface Dwell {
...
disruption — and builds back up once it rests. Under a finger it builds over `touchRise` instead, when given, since a touch rests only briefly.
A `pointercancel` — a touch the browser has taken over for
